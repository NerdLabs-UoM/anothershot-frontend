...
import { fetchHomeImageService } from "@/services/home/api";
...
const FeedImageComp = () => {
  ...
  useEffect(() => {
    const fetchFeedImages = async () => {
      try {
        const response = await fetchHomeImageService();
  ...
  const handleLike = async (
    ...
    try {
      ...
      const index = updatedFeedImages.findIndex(
        (image) => image.id === feedImageId
      );
      ...
      if (index !== -1) {
        updatedFeedImages[index] = {
          ...updatedFeedImages[index],
          likeCount: isLiked
            ? updatedFeedImages[index].likeCount - 1
            : updatedFeedImages[index].likeCount + 1,
          likedUserIds: isLiked
            ? updatedFeedImages[index].likedUserIds.filter(
      ...
      await axios.patch(
        `/api/photographer/${photographerId}/feed/like`,
        {
          feedId: feedImageId,
    ...
    try {
      ...
      if (index !== -1) {
        ...
        updatedFeedImages[index] = {
          ...updatedFeedImages[index],
          saveCount: isSaved
            ? updatedFeedImages[index].saveCount - 1
            : updatedFeedImages[index].saveCount + 1,
          savedUserIds: isSaved
            ? updatedFeedImages[index].savedUserIds.filter(
                (id) => id !== session.user.id
              )
      ...
      await axios.patch(`/api/photographer/${photographerId}/feed/createSave`, {
        feedId: feedImageId,
        userId: session?.user?.id,
        save: !isSaved,
      });
  ...
  const reportCreation = (id: string) => {
    if (
      session?.user?.userRole == "PHOTOGRAPHER" ||
        ...
        >
          <Masonry>
            {feedImages.map((feedImage: FeedImage) => (
              <div
                key={feedImage.id}
                className="relative flex flex-col justify-end items-center overflow-hidden w-[345px] h-auto lg:h-auto lg:w-[345px] rounded-[40px] mx-auto my-4"
              >
                <Image
                ...
                <div className="absolute z-10 grid grid-cols-6 items-center justify-center w-full bg-gradient-to-t from-black to-transparent rounded-b-[40px]">
                  <div
                    className="col-span-4 flex flex-start items-center justify-start pl-4 cursor-pointer"
                    onClick={() => handleRedirect(feedImage.photographerId)}
                  >
                    <Avatar className="w-14 h-14 sm:w-10 sm:h-10 lg:w-14 lg:h-14 border-2 border-white mr-3 hover:scale-110 transform transition duration-500">
                      <AvatarImage
                        src={feedImage.photographer.user.image ?? ""}
                        alt="@shadcn"
                      />
                      <AvatarFallback>CN</AvatarFallback>
                    </Avatar>
                    <div className="flex flex-col items-start justify-center">
                      <span className="text-white text-xs lg:text-md font-normal hover:scale-110 transform transition duration-500">
                        {feedImage.photographer.name}
                      </span>
                  ...
                  <div className="cursor-pointer">
                      ...
                      <div className="pl-4">
                        <Button
                          ...
                          onClick={() =>
                            handleLike(
                              feedImage.id,
                              feedImage.photographerId,
                              feedImage.likedUserIds != null &&
                                session !== null &&
                                feedImage.likedUserIds.includes(session.user.id)
                            )
                          }
                        >
                          {feedImage.likedUserIds != null &&
                    ...
                    <div className="flex flex-start items-center justify-between">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleSave(
                              feedImage.id,
                              feedImage.photographerId,
                              feedImage.savedUserIds != null &&
                                session !== null &&
                                feedImage.savedUserIds.includes(session.user.id)
                            )
                          }
                        >
                          {feedImage.savedUserIds != null &&
                      ...
                      <span className="text-white text-sm sm:text-xs lg:text-sm">
                        {formatCount(feedImage.saveCount)}
                      </span>
                    </div>
                  </div>
                </div>
