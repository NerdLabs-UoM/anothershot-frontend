...
export const NotifySection: React.FC<NotifyProps> = ({ logged }) => {
    ...
    const fetchNotify = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/notification/${session?.user?.id}`
        );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notification/delete/${id}/${userId}`
      );
  ...
  const handleRead = async (id: string) => {
    try {
      ...
      const res = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/notification/update`,
        { notifyId: id, read: true }
      );
  ...
  return (
    <Sheet>
      <SheetTrigger>
        {isReadAll ? (
          <Bell className="fill-black mb-8" />
        ) : (
