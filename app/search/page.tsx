...
import { fetchPhotographerCategories } from "@/services/photographer/categories/api";
import { searchPhotographers } from "@/services/home/api";
...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      setLoading(true);
      try {
        const res = await fetchPhotographerCategories();
        setCategories(
    ...
    const fetchPhotographers = async () => {
      try {
        const res = await searchPhotographers(undefined, undefined, undefined);
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    try {
      const res = await searchPhotographers(
        photographername,
        location,
        category
      );
                ...
                <div className="space-y-1">
                  <CardTitle className="flex align-middle">
                    <Avatar
                      ...
                      onClick={() =>
                        router.push(
                          `/user/photographer/${photographer.userId}/profile`
                        )
                      }
                    >
                      <AvatarImage
              ...
              <CardContent>
                <div className="text-sm text-foreground">
                  <div className="flex items-center">
                    ...
                    <span className="ml-2">
                      {(
                        photographer.testimonial.reduce(
                          (acc, curr) => acc + curr.rating,
                          0
                        ) / photographer.testimonial.length
                      ).toFixed(1) || "Newbie"}
                    </span>
                  </div>
                  <div className="flex items-center">
                    <MapPin size={16} />
                    <span className="ml-2">
                      {photographer.contactDetails?.address?.city}
                    </span>
                  </div>
