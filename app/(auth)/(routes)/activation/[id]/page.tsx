...
const ActivationPage = ({ params }: ActivationPageProps) => {
    ...
    const handleActivation = async () => {
        try {
            setLoading(true);
            const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/user/activate/${params.id}`);
            if (res.data.error) {
    ...
    return (
        <div className="grid text-center content-center h-full ">
            <div className="md:border rounded-[20px] md:p-10 min-w-[350px]">
                <h1 className="text-3xl font-bold m-4">AnotherShot Account Activation</h1>
                <Button variant="default" className="w-full" onClick={() => handleActivation()}>
