...
const fetchToken = async (userId: string, email: string) => {
  try {
    const res = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/api/auth/token/${userId}`,
      {
        email,
      }
    );
    ...
    async signIn({ user, account, profile, credentials }) {
      ...
      if (account?.provider === "google") {
        ...
        if (fetchedUser.accounts.length >= 0) {
          const accountExists = fetchedUser.accounts.find(
            (acc: any) => acc.provider === account.provider
          );
          if (!accountExists) {
            await axios.post(
              `${process.env.NEXT_PUBLIC_API_URL}/api/user/account/${fetchedUser.id}`,
              account
            );
          }
          return true;
